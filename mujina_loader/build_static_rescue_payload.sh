...
OUT_PAYLOAD="${SCRIPT_DIR}/payload-yocto-kernel-rescue"
WORK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/mujina-static-rescue.XXXXXX")"
...
usage() {
  cat <<EOF
Usage:
  ./build_static_rescue_payload.sh [options]
...
echo "Mujina rescue init started"
echo "Kernel: $(uname -a 2>/dev/null || true)"
echo "Cmdline: $(cat /proc/cmdline 2>/dev/null || true)"
echo "Mounts:"
mount 2>/dev/null || true
echo
echo "Attempting /usr/bin/bash.bash if present..."
if [ -x /usr/bin/bash.bash ]; then
  /usr/bin/bash.bash -lc 'echo bash-ok' || echo "bash exited rc=$?"
fi
echo
...
docker run --rm \
  ...
  bash -lc "
    ...
    apt-get install -y --no-install-recommends \
      ...
      gcc-aarch64-linux-gnu libc6-dev-arm64-cross make >/dev/null
    cd /tmp
    ...
    set +o pipefail
    yes "" | make ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- oldconfig >/dev/null
    set -o pipefail
    make -j\"\$(nproc)\" ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- busybox >/dev/null
...
rootfs_profile=static-busybox-rescue
