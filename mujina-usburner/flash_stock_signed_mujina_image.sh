...
FLASH_TOOL="${FLASH_TOOL:-${ROOT_DIR}/stock_fw_restore/flash-tool}"
UPDATE_BIN="${UPDATE_BIN:-${ROOT_DIR}/stock_fw_restore/tools/macos/update}"
IMAGE="${IMAGE:-${SCRIPT_DIR}/output/aml_upgrade_package_mujina_stock_signed.img}"
...
verify_inputs() {
  ...
  [[ -e /usr/local/opt/libusb-compat/lib/libusb-0.1.4.dylib ]] || die "Missing libusb-compat. Run stock_fw_restore/install_host_deps.sh first."
  need_cmd awk
}
